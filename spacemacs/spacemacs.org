...
** Indent mode
Load org files with indent mode enabled by default. This causes the content
within a heading to visually line up under the heading.
#+name: behavior
#+BEGIN_SRC emacs-lisp
...
editing buffer. Instead, let's configure org to edit in a new buffer split in
the current window.
#+name: behavior
#+BEGIN_SRC emacs-lisp
